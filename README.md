...
If you later add optional encrypted backup, it must be opt-in and user-owned storage (your S3 / your NAS). Do not ship a toggle that ships raw screenshots off-box.
...
app/main.py      FastAPI + /tools + /call_tool
app/store.py     SQLite
app/capture.py   screenshot + process list (mss / fallback)
~/.desktrace/    snapshots.db + shots/
```

Stack choice: Python + FastAPI so you can run it tonight on the Windows laptop. Tauri/Rust native shell is the v0.2 wrap when the capture loop is proven.
...
Browser tabs: v0.2. Reading live Chrome/Edge tab lists needs a companion extension or debug port — not silently scraping profile DBs.
